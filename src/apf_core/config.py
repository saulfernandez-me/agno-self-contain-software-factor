...
def load_apf_config(config_path: str = "apf.yaml") -> ApfConfig:
    """
    Loads and parses the APF configuration YAML file.
    Returns a strongly typed Pydantic ApfConfig object.
    ...
    try:
        with p.open("r", encoding="utf-8") as f:
            ...
            if not isinstance(data, dict):
                return ApfConfig()
            return ApfConfig.model_validate(data)
    except (OSError, yaml.YAMLError, ValueError):
        # If YAML is corrupted or validation fails, return defaults to prevent crashing
        return ApfConfig()


def get_models_for_tier(tier_name: str, config: ApfConfig | None = None) -> list[str]:
    """
    ...
    if tier_name == "heavy":
        return ["google:gemini-3.1-pro-preview", "deepseek:deepseek-reasoner", "anthropic:claude-3-5-sonnet"]
    elif tier_name == "workhorse":
        return ["google:gemini-3.5-flash", "deepseek:deepseek-chat", "openai:gpt-4o-mini"]
    else:  # lightweight or unknown
        return ["google:gemini-2.5-flash", "deepseek:deepseek-chat"]
