...
app = typer.Typer(help="ASF Command Line Interface - The Generative Toolkit")
...
def get_base_dir() -> Path:
    """Determine the base directory. If we are developing the framework, use templates/asf_scaffold."""
    cwd = Path.cwd()
    scaffold_dir = cwd / "templates" / "asf_scaffold"
...
"""
    ...
    runner_content = f"""from asf_core.workflow import AsfWorkflow  # type: ignore[import-not-found]
...
def run_{name}(task_description: str, domain_context: str) -> None:
    ...
    workflow = AsfWorkflow(name="{name}")
...
if __name__ == "__main__":
    run_{name}("Do the task", "You are in an ASF repository.")
"""
