...
class AsfWorkflow(Workflow):
    """
    ASF custom workflow extending Agno's Workflow.
    ...
    def run_agent(self, agent: Any, prompt: str) -> Any:
        ...
        for model in models_to_try:
            try:
                ...
                print(
                    f"[ASF] Cognitive Phase: Running {agent.name} with model {getattr(agent.model, 'id', getattr(agent.model, 'name', str(model)))}..."
                )
                ...
                print(
                    f"[ASF] Formatting Phase: Extracting Envelope via {getattr(formatter_model, 'id', str(formatter_model))}..."
                )
                formatter = Agent(
                    name="Formatter",
                ...
                print(
                    f"[ASF] Warning: {last_error} Falling back to next cognitive model..."
                )

            except (OSError, ValueError, RuntimeError, ImportError) as e:
                last_error = str(e)
                print(
                    f"[ASF] Warning: Model {getattr(agent.model, 'id', getattr(agent.model, 'name', str(model)))} raised exception: {last_error}. Falling back..."
                )
