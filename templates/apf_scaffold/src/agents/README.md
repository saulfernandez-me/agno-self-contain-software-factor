...
Instead of creating dozens of hyper-specific agents (e.g., `frontend_dev`, `database_admin`, `qa_engineer`), APF relies on **7 Universal Archetypes**. Their physical capabilities (Tools) and base cognitive power (Model Tiers) are hardcoded in these Python files, ensuring the **Principle of Least Privilege**.
