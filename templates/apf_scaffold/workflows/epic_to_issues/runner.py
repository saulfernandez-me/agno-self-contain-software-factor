...
def run(epic_description: str, domain_context: str, milestone_id: str = "") -> None:
    ...
    with workflow.lane("code"):
        ...
        if milestone_id:
            ...
            _, repo_name, _ = run_shell_command("gh repo view --json nameWithOwner --jq .nameWithOwner")
            if repo_name:
                run_shell_command(f'gh api -X PATCH repos/{repo_name}/milestones/{milestone_id} -f title="[Epic] {backlog_envelope.epic_title}"')
        else:
