# Cookbook 03: Updating the APF Configuration (apf.yaml)

> **Purpose:** Instructs autonomous agents and engineers on how to safely mutate the `apf.yaml` hardware configuration file without breaking the 3-Tier model architecture.

## 1. The 3-Tier Model Architecture
APF does not map models to specific agents (e.g., "Planner gets GPT-4"). Instead, it maps models to **Complexity Tiers**.
...
When instructed to "change the model" or "add a fallback", you must manipulate the `apf.yaml` file located in `.context/`.
