# Cookbook 01: Creating an APF Workflow

> **Purpose:** This guide teaches autonomous agents and engineers how to create a new APF Workflow using the dual Graph-to-Code paradigm (Mermaid.js + Python ApfWorkflow).

## 1. The Mermaid Blueprint (`.mermaid`)

Every workflow in APF must be visually defined using Mermaid.js before it is executed. You must strictly use `subgraph` blocks to partition nodes into execution lanes.
...
def run_feature_workflow(task_description: str):
    workflow = ApfWorkflow(name="feature_implementation")
