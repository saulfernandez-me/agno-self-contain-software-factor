# Cookbook 02: Creating an Agno Agent for ASF

> **Purpose:** This guide dictates how to instantiate and configure an Agno `Agent` so it complies with the strict physical contract boundaries (Pydantic Envelopes) of ASF.
...
### Rules for the Agent:
1. **Model Agnostic:** The LLM provider (OpenAI, Anthropic, Gemini) should be configurable via `asf.yaml`, not hardcoded.
...
from asf_core.envelopes import EnvelopeBase
...
def get_planner_agent(session_id: str | None = None) -> Agent:
    return Agent(
        name="Planner",
        model=OpenAIChat(id="gpt-4o"),  # In production, read from asf.yaml
        ...
        output_schema=PlannerEnvelope,  # MANDATORY in ASF
