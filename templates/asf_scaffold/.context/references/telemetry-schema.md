...
> **Purpose:** Explains the internal SQLite database structure of ASF. Agents can use this knowledge to answer financial, performance, or diagnostic questions from the human engineer.

## 1. The WAL-Mode SQLite Database
All ASF executions are traced in real-time to a local SQLite database located at `.context/data/telemetry.db`.
...
**Do not attempt to modify (INSERT/UPDATE) this database.** It is managed exclusively by the `AsfWorkflow` Python engine.
