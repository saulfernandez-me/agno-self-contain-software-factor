# Cookbook 01: Creating an ASSF Workflow

> **Purpose:** This guide teaches autonomous agents and engineers how to create a new ASSF Workflow using the dual Graph-to-Code paradigm (Mermaid.js + Python AssfWorkflow).

## 1. The Mermaid Blueprint (`.mermaid`)

Every workflow in ASSF must be visually defined using Mermaid.js before it is executed. You must strictly use `subgraph` blocks to partition nodes into execution lanes.
...
Once the blueprint exists, create the Python orchestration script using the `AssfWorkflow` class from `assf_core.workflow`.
