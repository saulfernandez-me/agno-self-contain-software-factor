...
> **Purpose:** Explains the internal SQLite database structure of ASSF. Agents can use this knowledge to answer financial, performance, or diagnostic questions from the human engineer.

## 1. The WAL-Mode SQLite Database
All ASSF executions are traced in real-time to a local SQLite database located at `.context/data/telemetry.db`.
...
- `name` (TEXT): The name of the workflow (e.g., `standard_sdlc`).
...
**Do not attempt to modify (INSERT/UPDATE) this database.** It is managed exclusively by the `AssfWorkflow` Python engine.
