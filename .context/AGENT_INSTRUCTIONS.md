---
name: asf
description: The Agno Self-Contain Software Factor (ASF) super-skill. This skill contains the mandatory operational rules, architectural pillars, and routing tables for all agents working in an ASF-stamped repository. Always consult this skill to understand how to write workflows, agents, and data envelopes.
---

# 🏭 ASF: Agno Self-Contain Software Factor

> **CRITICAL DIRECTIVE:** You are operating inside an ASF-stamped repository. You MUST adhere to these rules at all times. Failure to do so breaks the software factory.
...
2. **Graph Sovereignty:** Do not attempt to write free-form agent loops. The workflow graph is owned by deterministic Python (`AsfWorkflow`). Agents are bounded nodes invoked by the Python code.
3. **Execution Lanes:** 
...
When asked to perform a specific ASF task, refer to the following patterns (which will be defined in the `docs/` or `templates/` folder of this repository):

| Request | Action / Pattern to Follow |
| :--- | :--- |
| **"Create a new contract/envelope"** | Inherit from `EnvelopeBase` in `asf_core.envelopes`. Define strict Pydantic fields. |
...
## 3. 🧠 Agent Integration
By having this skill loaded, you inherently know how ASF works. When reviewing code, generating Mermaid graphs, or writing Python for this repository, you must apply the 5 Pillars of ASF described above.
