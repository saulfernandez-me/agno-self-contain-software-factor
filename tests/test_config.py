...
from apf_core.config import ApfConfig, get_models_for_tier, load_apf_config


def test_load_apf_config_defaults(tmp_path: Path) -> None:
    """Test that missing config returns safe defaults."""
    missing_path = tmp_path / "does_not_exist.yaml"
    config = load_apf_config(str(missing_path))
    assert isinstance(config, ApfConfig)
    assert config.domain.name == "unknown_domain"
    assert config.limits.correction_loop_max_attempts == 3


def test_load_apf_config_valid(tmp_path: Path) -> None:
...
limits:
  correction_loop_max_attempts: 5
    ...
    config = load_apf_config(str(config_file))
