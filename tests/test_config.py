...
from assf_core.config import AssfConfig, get_models_for_tier, load_assf_config


def test_load_assf_config_defaults(tmp_path: Path) -> None:
    """Test that missing config returns safe defaults."""
    missing_path = tmp_path / "does_not_exist.yaml"
    config = load_assf_config(str(missing_path))
    assert isinstance(config, AssfConfig)
    assert config.domain.name == "unknown_domain"
    assert config.limits.correction_loop_max_attempts == 3


def test_load_assf_config_valid(tmp_path: Path) -> None:
...
limits:
  correction_loop_max_attempts: 5
    """
    config_file = tmp_path / "assf.yaml"
    config_file.write_text(yaml_content)

    config = load_assf_config(str(config_file))
